...
export default function TopicsFullHistory() {
    ...
    const { allHistory, refreshHistory, setSelectedHistoryItem } = useTopicHistoryStore();
    ...
    const filteredHistory = allHistory.filter((item: TopicHistoryItem) => {
        // Search filter
        if (filters.searchTerm) {
            const searchLower = filters.searchTerm.toLowerCase();
            if (!item.topic.toLowerCase().includes(searchLower) && 
        ...
        // Detail level filter
        if (filters.detailLevel !== 'all' && item.detailLevel !== filters.detailLevel) {
            return false;
        }
        ...
        return true;
    }).sort((a: TopicHistoryItem, b: TopicHistoryItem) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    ...
    const getStatsData = () => {
        ...
        return {
            ...
            mostUsedLevel: Object.entries(detailLevelCounts).reduce((a, b) => 
                detailLevelCounts[a[0] as keyof typeof detailLevelCounts] > detailLevelCounts[b[0] as keyof typeof detailLevelCounts] ? a : b
            )[0]
        };
    };
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 sm:p-6">
            <div className="max-w-6xl mx-auto space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    {[
                        { icon: BookOpen, label: 'Total Topics', value: stats.totalItems, color: 'text-blue-500' },
                        { icon: FileText, label: 'Brief Explanations', value: stats.detailLevelCounts.less, color: 'text-green-500' },
                        { icon: BookOpen, label: 'Detailed Explanations', value: stats.detailLevelCounts.more, color: 'text-amber-500' },
                        { icon: Layers, label: 'Comprehensive', value: stats.detailLevelCounts.most, color: 'text-blue-500' }
                    ].map((stat, index) => (
                ...
                <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                        ...
                        {(filters.detailLevel !== 'all' || filters.dateRange !== 'all' || filters.searchTerm) && (
                            <button
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => setFilters(prev => ({ ...prev, detailLevel: e.target.value as FilterState['detailLevel'] }))}
                    ...
                    {filteredHistory.length > 0 ? (
                        <div className="space-y-4">
                            {filteredHistory.map((item: TopicHistoryItem) => (
                                ...
                                >
                                    <div className={`p-4 rounded-xl border-2 ${getDetailLevelBgColor(item.detailLevel)} hover:shadow-md transition-shadow duration-200`}>
                                        {/* Header Section */}
