...
export const QuizQnaFullHistory = () => {
    ...
    const { allHistory, refreshHistory, setSelectedHistoryItem } = useQuizQnAHistoryStore();
    ...
    const filteredHistory = allHistory.filter((item: HistoryItem) => {
        // Type filter
        if (filters.type !== 'all' && item.type !== filters.type) return false;
        
        // Performance filter
        if (filters.performance !== 'all') {
            const percentage = item.type === 'quiz' 
                ? item.result?.percentage || 0 
                : item.result?.totalScore && item.result?.maxPossibleScore 
                    ? Math.round((item.result.totalScore / item.result.maxPossibleScore) * 100) 
                    : 0;
        ...
        // Date range filter
        if (filters.dateRange !== 'all' && !isWithinDateRange(item.timestamp, filters.dateRange)) {
            return false;
        }
    ...
    const getStatsData = () => {
        const quizItems = filteredHistory.filter((item: HistoryItem) => item.type === 'quiz');
        const qnaItems = filteredHistory.filter((item: HistoryItem) => item.type === 'qna');
        
        const avgQuizScore = quizItems.length > 0 
        ...
        const avgQnaScore = qnaItems.length > 0 
            ? qnaItems.reduce((sum: number, item: HistoryItem) => {
                const qnaItem = item as QnAHistoryItem;
                const percentage = qnaItem.result?.totalScore && qnaItem.result?.maxPossibleScore 
                    ? Math.round((qnaItem.result.totalScore / qnaItem.result.maxPossibleScore) * 100) 
                    : 0;
                return sum + percentage;
            }, 0) / qnaItems.length 
            : 0;
        ...
        return {
            ...
            overallAvg: filteredHistory.length > 0 
                ? Math.round(filteredHistory.reduce((sum: number, item: HistoryItem) => {
                    const percentage = item.type === 'quiz' 
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 sm:p-6">
            <div className="max-w-6xl mx-auto space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    {[
                        { icon: Trophy, label: 'Total Sessions', value: stats.totalItems, color: 'text-yellow-500' },
                        { icon: BookOpen, label: 'Quiz Average', value: `${stats.avgQuizScore}%`, color: 'text-blue-500', score: stats.avgQuizScore },
                        { icon: Brain, label: 'Q&A Average', value: `${stats.avgQnaScore}%`, color: 'text-purple-500', score: stats.avgQnaScore },
                        { icon: TrendingUp, label: 'Overall Average', value: `${stats.overallAvg}%`, color: 'text-green-500', score: stats.overallAvg }
                    ].map((stat, index) => (
                        <div key={index} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow duration-200">
                            ...
                            <div className={`text-2xl font-bold ${stat.score !== undefined ? getScoreColor(stat.score) : 'text-gray-900 dark:text-white'}`}>
                                {stat.value}
                            </div>
                        </div>
                    ))}
                ...
                <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                        ...
                        {(filters.type !== 'all' || filters.performance !== 'all' || filters.dateRange !== 'all' || filters.searchTerm) && (
                            <button
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => setFilters(prev => ({ ...prev, performance: e.target.value as FilterState['performance'] }))}
                                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                                >
                ...
                <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
                        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
                            History <span className="text-blue-600 dark:text-blue-400">({filteredHistory.length})</span>
                        </h2>
                        ...
                        <div className="grid gap-4">
                            {filteredHistory.map((item: HistoryItem) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className={`p-4 rounded-xl border-2 ${getScoreBgColor(percentage)} hover:shadow-md transition-shadow duration-200`}>
                                            {/* Header Section */}
                                            <div className="flex items-start justify-between mb-3">
                                                <div className="flex items-center gap-3 min-w-0 flex-1 w-[100]">
                                                    ...
                                                    <div className="min-w-0 flex-1  ">
                                                        <h3 className="font-semibold text-gray-900 dark:text-white text-lg truncate  mb-1" title={item.topic}>
                                                            {item.topic}
                                                        </h3>
                                                        <p className="text-sm text-gray-600 dark:text-gray-400 truncate" title={item.educationLevel}>
                                                            {item.educationLevel}
                                                        </p>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <div className={`p-1 rounded-md ${getScoreColor(percentage).replace('text', 'bg')} bg-opacity-10`}>
                                                    ...
                                                    <div className="text-xs text-gray-500 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-md">
                                                        {item.type === 'quiz' 
                                                            ? `${(item as QuizHistoryItem).result?.score || 0}/${(item as QuizHistoryItem).result?.totalQuestions || 0} correct`
                                                            : `${(item as QnAHistoryItem).result?.totalScore || 0}/${(item as QnAHistoryItem).result?.maxPossibleScore || 0} points`
                                                        }
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-2 text-xs text-gray-500">
                                                    ...
                                                    <span className="hidden sm:inline" title={formatDate(item.timestamp)}>
                                                        {formatDate(item.timestamp)}
                                                    </span>
                                                    <span className="sm:hidden" title={formatDate(item.timestamp)}>
